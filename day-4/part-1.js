...
const passphraseDump = `bdwdjjo avricm cjbmj ran lmfsom ivsof
...
const howManyPassphrasesValid = dump => {
  const isUnique = (searchWord, otherWords) => otherWords.findIndex(word => word === searchWord) < 0;

  const passphrases = passphraseDump
    .split(/\n/)
    .map(phrase => phrase.split(" "));

  const passphraseToUniqueWords = passphrase => passphrase.map((word, ind, arr) => 
    isUnique(word, arr.slice(0, ind).concat(arr.slice(ind + 1))));

  const everyElementTrue = row => row.every(wordBool => wordBool);

  const uniqueWordsToUniquePassphrases = passphrases => passphrases
    .map(passphraseToUniqueWords)
    .map(everyElementTrue);

  const count = uniqueWordsToUniquePassphrases(passphrases)
    .reduce((acc, curr) => curr ? acc + 1 : acc);

  return count;
}

console.log(`there are ${howManyPassphrasesValid(passphraseDump)}`);